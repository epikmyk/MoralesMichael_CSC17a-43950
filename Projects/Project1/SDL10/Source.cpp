...
int main(int argc, char* args[])
{
	...
	Setting door(gameRenderer, "Images/door.png", 10500, 0, 345, 720, &cameraX, &cameraY);
	//sign for password hint
	Setting sign(gameRenderer, "Images/sign.png", 9800, 410, 200, 200, &cameraX, &cameraY);
	...
	while (gameRunning)
	{
		...
		if (gameStart)
		{
			...
			if (!mainMenuScreen)
			{
				...
				//draw hail sign
				hailSign.drawSetting(gameRenderer, &cameraX);
				//draw mountains
				mountains.drawSetting(gameRenderer, &cameraX);
				//draw door exit to finish game
				door.drawSetting(gameRenderer, &cameraX);
				//draw sign for password hint
			...
			if (!shadow.collide(ground) && !shadow.collide(ground2) &&
				...
				!shadow.collide(ground5) &&  shadow.getOriginY() > 500)
			{
